					...
					if(result[0].role<2){
						if(1==1){
							if(req.body.filial!=req.session.filial && req.session.role>0){
								res.end(JSON.stringify({status:'fail',body:'Недостаточно прав. Вы не можете управлять пользователями другого филиала'}));
								return;
							};
							if(req.body.role==0 && req.session.role==1){
								res.end(JSON.stringify({status:'fail',body:'Недостаточно прав. Вы не можете создать суперадмина'}));
								return;
							};		
							collection.insertOne({
								...
								if(err){
									res.end(JSON.stringify({status:'fail',body:'Ошибка, пользователь с таким логином уже существует'}));
								}else{
									sendmail({
						...
						res.end(JSON.stringify({status:'fail',body:'Недостаточно прав, вы не можете создавать пользователей'}));
					};
				};
			});
		});
					...
					if(err){
						...
						if(result[0].filial===req.session.filial || req.session.role===0){
							collection.deleteOne({login:req.body.login}, function(err, resourse) {
								if(err){
									res.end(JSON.stringify({status:'fail',body:'Ошибка БД удаления пользователя'}));
								}else{
									res.end(JSON.stringify({status:'success'}));
								};
							});			
						}else{
							res.end(JSON.stringify({status:'fail',body:'Недостаточно прав, вы не можете удалять пользователей другого филиала'}));
						};
					};
				});
			});
...
var getuser=function(req,res){
	if(req.session.role<2){
					...
					if(err){
						...
						if(document){
							if(document.filial===req.session.filial || req.session.role===0){
								...
								res.end(JSON.stringify({status:'fail',body:'Недостаточно прав, вы не можете редактировать пользователей другого филиала'}));
							};
						}else{
		...
		res.end(JSON.stringify({status:'fail',body:'Недостаточно прав, вы не можете редактировать пользователей'}));
	};
};
...
var getphotoreport=function (req,res){
	...
	if(req.session.role===0){
		query={date: {$gte:new Date(moment(dates[0],'DD.MM.YYYY').startOf('day')),$lte: new Date(moment(dates[1],'DD.MM.YYYY').add(1,'day').startOf('day'))}, region: {$exists: true}, status:{$in:req.body.status}}
	}else{
		query={date: {$gte:new Date(moment(dates[0],'DD.MM.YYYY').startOf('day')),$lte: new Date(moment(dates[1],'DD.MM.YYYY').add(1,'day').startOf('day'))}, $or: [{region:req.session.filial},{region:Number(req.session.filial)}], status:{$in:req.body.status}}	
	};
	if(req.session.role===2){
		if(req.session.places){
			query={date: {$gte:new Date(moment(dates[0],'DD.MM.YYYY').startOf('day')),$lte: new Date(moment(dates[1],'DD.MM.YYYY').add(1,'day').startOf('day'))}, $or: [{region:req.session.filial},{region:Number(req.session.filial)}], status:{$in:req.body.status},city:{$in:req.session.places}}	
		}else{
			query={date: {$gte:new Date(moment(dates[0],'DD.MM.YYYY').startOf('day')),$lte: new Date(moment(dates[1],'DD.MM.YYYY').add(1,'day').startOf('day'))}, $or: [{region:req.session.filial},{region:Number(req.session.filial)}], status:{$in:req.body.status}}	
		};	
	};
	if(req.session.role===3){
		query={region:req.session.filial, status:{$in:status}, $or:[{worksite:{$in:req.session.places}},{chieflogin:req.session.login}]}
	};
...
var getchiefs=function (req,res){
	...
	if(req.session.role<3){
		client.connect(dburl,function(err, client) {
			const db = client.db(dbName);
			db.collection('photo_users', function (err, collection) {
				collection.find({filial:req.session.filial, role:3, places: {$in:[req.body.worksite]}}).toArray(function(err, result) {
					if(err){
						res.end(JSON.stringify({status:'fail',body:err}));
					}else{
						if(result.length){
							...
							collection.find({filial:req.session.filial, role:3}).toArray(function(err, result2) {
								if(err){
									...
									if(result2.length){
										...
										res.end(JSON.stringify({status:'fail',body:'Не найдено ответственных для данного участка'}));
									};
								};
							});
						};
		...
		res.end(JSON.stringify({status:'fail',body:'Недостаточно прав. Для повышения прав обратитесь к администратору РФ'}));
	};
};
...
var claimtask=function (req,res){
	if(req.session.role<3){
		client.connect(dburl,function(err, client) {
			const db = client.db(dbName);
			db.collection('photos', function (err, collection) {
				collection.updateMany({taskname:req.body.taskname}, {$set: {status:'Претензия', chiefname: req.body.chiefname, chieflogin: req.body.chieflogin, claimtext: req.body.claimtext, claimedname: req.session.login}}, function(err, r) {
					if(err){
						res.end(JSON.stringify({status:'fail',body:err}));
					}else{					
...
var accepttask=function (req,res){
	if(req.session.role<3){
		client.connect(dburl,function(err, client) {
			const db = client.db(dbName);
			db.collection('photos', function (err, collection) {
				collection.updateMany({taskname:req.body.taskname}, {$set: {status:'Подтверждён', acceptedlogin: req.session.login}}, function(err, r) {
					if(err){
						res.end(JSON.stringify({status:'fail',body:err}));
					}else{
						res.end(JSON.stringify({status:'success'}));
...
var setcomment=function (req,res){
	client.connect(dburl,function(err, client) {
		const db = client.db(dbName);
		db.collection('photos', function (err, collection) {
			collection.updateMany({taskname:req.body.taskname}, { $addToSet: {comments: {text:req.body.comment, commentlogin: req.session.login, commentname: req.session.name,commentdate: new Date()} } }, function(err, r) {
				if(err){
					res.end(JSON.stringify({status:'fail',body:err}));
				}else{				
					collection.findOne({taskname:req.body.taskname}, function(err, document) {
...
var fixtask=function (req,res){
	client.connect(dburl,function(err, client) {
		const db = client.db(dbName);
		db.collection('photos', function (err, collection) {
			collection.updateMany({taskname:req.body.taskname}, {$set: {status:'Доработан',fixdate:new Date(),fixlogin:req.session.login}}, function(err, r) {
				if(err){
					res.end(JSON.stringify({status:'fail',body:err}));
				}else{
					res.end(JSON.stringify({status:'success'}));
								...
								if(document){
									...
									function(err, resourse) {
										if (err){ 
											...
											db.collection('photos').findOne({taskname:req.body.taskname}, function(err, document) {
												res.end(JSON.stringify({status:'success',body:document.photos}));
												db.close();
											}); 
										};							
									});									
...
var getcsv=function(req,res){
	...
	var query;
	if(req.session.role===0){
		query={date: {$gte:new Date(moment(dates[0],'DD.MM.YYYY').startOf('day')),$lte: new Date(moment(dates[1],'DD.MM.YYYY').add(1,'day').startOf('day'))}, region: {$exists: true}}
	}else{
		query={date: {$gte:new Date(moment(dates[0],'DD.MM.YYYY').startOf('day')),$lte: new Date(moment(dates[1],'DD.MM.YYYY').add(1,'day').startOf('day'))}, $or: [{region:req.session.filial},{region:Number(req.session.filial)}]}	
	};
	if(req.session.role===2){
		query={date: {$gte:new Date(moment(dates[0],'DD.MM.YYYY').startOf('day')),$lte: new Date(moment(dates[1],'DD.MM.YYYY').add(1,'day').startOf('day'))}, $or: [{region:req.session.filial},{region:Number(req.session.filial)}], city:{$in:req.session.places}}	
	};	
	if(req.session.role===3){
		query={$or: [{region:req.session.filial},{region:Number(req.session.filial)}], chieflogin: req.session.login, status:'Претензия'}
	};
	client.connect(dburl,function(err, client) {
		const db = client.db(dbName);
		db.collection('photos', function (err, collection) {
					...
					db.collection('worksite', function (err, collection) {
						collection.find({REGION:Number(req.body.region)}).toArray(function(err, worksites) {
							if(err){
								...
								res.end(JSON.stringify({status:'success',worksites:worksites,regions:regions}));
							};
						});				
					});					
				};
...
var getphotoscount=function (req,res){
	oracledb.getConnection(
	  ...
	  function(err, connection)
	  {
		if (err) { 
			console.error(err);
			res.end(JSON.stringify({status:'fail',body:'БД Аргус временно недоступна, просмотр фотографий невозможен. Попробуйте позже.'}));
			return; 
		}
		connection.execute(
			`	select 
		  ...
		  {
			if(err){
				res.end(JSON.stringify({status:'fail',body:'Ошибка, обратитесь к администратору или попробуйте позже.'}));
			}else{
...
var getmaterials=function (req,res){
	...
	switch (req.session.filial){
		case 86:filial='Ханты-Мансийский филиал';break;
		case 66:filial='Екатеринбургский филиал';break;
		case 59:filial='Пермский филиал';break;
		case 74:filial='Челябинский филиал';break;
		case 72:filial='ФТК';break;
		case 89:filial='Ямало-Ненецкий филиал';break;
	}
	if(req.session.role===0)
		filial={ $exists: true };	
	...
	MongoClient.connect(url, function(err, db) {
		  if (err) throw err;
		  var dbo = db.db("Tasks");
		  dbo.collection("materials").find({date: {$gte:new Date(moment(dates[0],'DD.MM.YYYY').startOf('day')),$lte: new Date(moment(dates[1],'DD.MM.YYYY').add(1,'day').startOf('day'))},filial:filial}).toArray(function(err, items) {
			if (err) throw err;
			var content = 
			  'Филиал'+'\t'+
			  'Дата'+'\t'+
