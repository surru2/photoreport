...
function showPhoto(task, index, anchor) {
    ...
    photoBody += `</p>`;
    if (userRole < 3) {
        photoBody += `<a class="btn btn-primary" href="#" onclick="updateTaskDiag(tasks[` + index + `],'` + anchor + `',` + index + `);"><i class="fa fa-thumbs-o-down" aria-hidden="true"></i>&nbsp;Претензия</a>`;
        photoBody += `<a class="btn btn-primary mx-1" href="#" onclick="acceptTask('` + task.taskname + `','` + anchor + `');"><i class="fa fa-thumbs-o-up" aria-hidden="true"></i>&nbsp;Подтвердить</a>`;
    ...
    ;
    photoBody += `<a class="btn btn-primary mx-1" href="#" onclick="$('#maincontainer').hide();$('#getPhotoReportG').show();$('html, body').animate({ scrollTop: $('#${anchor}').offset().top}, 5);"><i class="fa fa-times" aria-hidden="true"></i>&nbsp;Закрыть</a>`;
    photoBody += `</p>
			</div>
		</div>	
    ...
    photoBody += `
				...
				<div class="btn-group dropright">
					<span class="btn btn-primary" href="#" onclick="setComment(` + index + `);">Отправить</span>
				</div>
			</div>	
		</div>
...
function acceptTask(taskname, anchor) {
    ...
    $.post("/main", { act : 'accepttask', taskname : taskname }, function (data) {
        try {
            ...
            if (data.status === 'success') {
                ...
                $('#' + anchor.replace('btn', 'sts')).html('Подтверждён')
                $('html, body').animate({ scrollTop : $('#' + anchor).offset().top }, 5);
                // getPhotoReport();
...
function fixTask(taskname, anchor) {
    $.post("/main", { act : 'fixtask', taskname : taskname }, function (data) {
        try {
            ...
            if (data.status === 'success') {
                ...
                $('#getPhotoReportG').show();
                $('#maincontainer').hide();
                $('#' + anchor.replace('btn', 'sts')).html('Доработано');
                $('html, body').animate({ scrollTop : $('#' + anchor).offset().top }, 5);
            } else {
                $("#photoreporttableinfo").html(data.body);
                jrumbleitem($("#photoreporttableinfo"));
...
function claimTask(anchor, index) {
    $.post("/main", {
        act : 'claimtask',
        taskname : $('#claimtaskname').html(),
        chieflogin : $('#claimchief option:selected').val(),
        chiefname : $('#claimchief option:selected').text(),
        ...
        try {
            ...
            data = JSON.parse(data);
            if (data.status === 'success') {
                tasks[index].status = 'Претензия';
                tasks[index].claimtext = $('#claimtext').val();
                tasks[index].chieflogin = $('#claimchief option:selected').val();
                tasks[index].chiefname = $('#claimchief option:selected').text();
                $('#getPhotoReportG').show();
                $('#maincontainer').hide();
                $('#' + anchor.replace('btn', 'sts')).html('Претензия');
                $('#' + anchor.replace('btn', 'chf')).html($('#claimchief option:selected').text());
                $('html, body').animate({ scrollTop : $('#' + anchor).offset().top }, 5);
    ...
    $.post("/main", query, function (data) {
        try {
            ...
            for (let i = 0; i < data.body.length; i++) {
                ...
                tablebody += `				  <td id="` + tasks[i].taskname + `sts">` + data.body[i].status + `</td>`;
                if (data.body[i].chiefname === undefined) data.body[i].chiefname = '';
                tablebody += `				  <td id="` + tasks[i].taskname + `chf">` + data.body[i].chiefname + `</td>`;
                tablebody += `
						<td>
							<div class="btn-group dropleft" style="">
								<button class="btn btn-primary mx-1" id="` + tasks[i].taskname + `btn" onclick="showPhoto(tasks[` + i + `],` + i + `,this.id)"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>											
								</button>
							</div>
						</td>
					`;
...
function updateTaskDiag(task, anchor, index) {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                showUserBody += `
							  ...
							  <div class="btn-group dropright">
								<a class="btn btn-primary" href="#" onclick="claimTask('` + anchor + `',` + index + `);">Отправить</a>
							  </div><a class="btn btn-primary mx-1" href="#" onclick="$('#maincontainer').hide();$('#getPhotoReportG').show();$('html, body').animate({ scrollTop: $('#${anchor}').offset().top}, 5);">Отмена</a>
							</div>
						  </div>
						</div>
					  </div>
